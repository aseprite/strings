...
def save_ini(
    ...
    with file_path.open('w', encoding='utf-8') as file:
        ...
        for section_name, src_section in src_ini.items():
            section = ini.get(section_name, None)
            file.write(f'\n[{section_name}]\n')
            for key, src_value in src_section.items():
                value = None if section is None else section.get(key, None)
                if value is None:
                    file.write(f'# TODO # {key} = {src_value}\n')
                else:
...
def main():
    ...
    for file_path in project_root_dir.iterdir():
        ...
        save_ini(headers, lang_ini, en_ini, file_path)
